...
### C. Insurance Dashboard
Located in `lib/screens/insurance_screen.dart`.
- **Active Plan Card:** Uses a high-contrast state (Active/Inactive) with a "Weekly Cycle" metric.
- **Benefit Rows:** Icon-rich, color-coded rows that explain coverage (Income, Weather, Strike) at a glance.
- **Manual Claim Section:** A dedicated fallback UI for when auto-detection fails, featuring an "Info Alert" style component for guidance.
